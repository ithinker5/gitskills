...
$(function () {
  ...
  function getTask(id) {
    service.getById(id, function (res) {
      ...
      var html2 = '<div><p><label>验收人：</label><span>' + (res.acceptance ? res.acceptance : res.created.name) + '</span></p></div>' + '<div><p><label>验收结果：</label><span>失败</span></p></div>';
      if (trialTask && trialTask == 'true') {
        ...
        html2 += '<div>' + '<p><label>悬赏V&nbsp;C：</label><span class="base_vc">' + res.base_vc + '</span></p>' + '<p><label>扣除V&nbsp;C：</label><span class="off_vc">' + (parseInt(res.status) === 14 ? res.base_vc : res.off_vc) + '</span></p>' + '<p><label>奖励V&nbsp;C：</label><span class="extra_vc">' + res.extra_vc + '</span></p>' + '</div>' + '<div>' + '<p><label>悬赏金币：</label><span class="base_gold">' + res.gold + '</span></p>' + '<p><label>扣除金币：</label><span class="off_gold">' + (parseInt(res.status) === 14 ? res.gold : res.off_gold) + '</span></p>' + '<p><label>奖励金币：</label><span class="extra_gold">' + res.extra_gold + '</span></p>' + '</div>' + '<div class="oneItem"><p><label>验收说明：</label><span>' + (res.check_desc ? res.check_desc : '无') + '</span></p></div>';
      }
