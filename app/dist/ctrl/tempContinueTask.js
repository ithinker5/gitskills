...
$(function () {
  ...
  var selectName = window.sessionStorage.getItem('selectLeaderName');
  var selectLeaderGold = window.sessionStorage.getItem('selectLeaderGold');
    ...
    service.getById(id, function (res) {
      ...
      if (res.applicant && res.applicant.user && parseInt(res.applicant.user._id) !== userId) {
        ...
        if (!faction_id) {
          ...
          for (var _i = 0; _i < res.taskApplicants.length; _i++) {
            if (res.taskApplicants[_i].user._id == userId) {
              if (res.taskApplicants[_i].mortgage > 0) {
          ...
          for (var _i2 = 0; _i2 < res.taskApplicants.length; _i2++) {
            if (res.taskApplicants[_i2].user._id == userId) {
              if (res.taskApplicants[_i2].mortgage > 0) {
                $("#footer").append('<input id="quitTask" class="btn btnStyle subBtn2 quitPartner" type="button" value="退出任务"/>');
              } else {
                $("#footer").append('<input id="quitTask" class="btn btnStyle subBtn2" type="button" value="退出任务"/>');
              }
  ...
  function zPopup(title, type, con, clickBtn, id, user_id) {
    ...
    $(".quitTask").off("click").on("click", function () {
      window.parent.playClickEffect();
      service.quitTask(id, function (res) {
        if (res.code == 400) {
          var _title = "提示";
          var _type = "2";
          var _con = res.msg;
    ...
    $(".confirmSubmitTask").off("click").on("click", function () {
      window.parent.playClickEffect();
      var obj = { status: 6, complete_desc: $('#finishDesc').val() };
      if (fileArr.length > 0) {
        obj.assets = fileArr;
      }
    ...
    $(".submitChangeLeader").off("click").on("click", function () {
      window.parent.playClickEffect();
      service.getTaskPerson(id, { user_id: $('input[name="applicant_id"]:checked').val() }, function (res) {
        if (res.code == 400) {
          var _title4 = "提示";
          var _type4 = "2";
          var _con4 = res.msg;
    ...
    function getCreateTeams() {
      ...
      service.getCreateTeams(name, function (res) {
        ...
        service.getMyTeams(function (res2) {
          var html = '<div class="editTeamBox editTeamBox2">' + '<p><label>队伍名称：</label><span id="teamName" data-team=' + res2[0]._id + '>' + res2[0].name + '</span><label>' + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;领队：<span id="teamLeader">' + res2[0].created.name + '</span><i>押金：</i><input class="goldInput" id="LeaderGold" /></p><p id="test2">队员：' + '<div class="recruit2"></div>' + '</div>';
          html += '</div>';
          $(".tableBox").append(html);
          for (var j = 0; j < res2[0].members.length; j++) {
    ...
    service.getUserJoin(id, userId, function (res) {
      if (res.isApply === true || res.isApply === 'true') {
        ...
        for (var i = 0; i < res.teams.length; i++) {
          var html = '<p><label>队伍名称：</label><span id="teamName" data-team=' + res.teams[i]._id + '>' + res.teams[i].name + '</span><label>' + '领队：</label><span id="teamLeader">' + res.teams[i].created.name + '</span>';
          html += '</p>' + '<div style="position:relative"><table id="accountTable' + i + '" class="table table-striped"></table>';
          ...
          if (i === 0) {
            (function () {
              var $joinMembers = res.teams[0].members;
        ...
        if (selectLeaderId || parseInt(selectLeaderId) === 0) {
          ...
          $('.EditTeam').css('display', 'none');
          $('.changeBoxTask .memberInfo .dataTables_wrapper').css('margin-bottom', '0');
          $('#applicant_id').val('' + window.sessionStorage.getItem('selectId')).attr({ 'data-name': window.sessionStorage.getItem('selectLeaderName'), 'data-gold': selectLeaderGold });
  ...
  $(document).on('click', '#submitChangeLeader', function () {
    ...
    if ($('input[name="applicant_id"]:checked').data('name')) {
      ...
      if (error) {
        ...
        var clickBtn = 'submitChangeLeader';
        zPopup(title, type, con, clickBtn, id);
      }
    } else {
      var _title5 = "提示";
  ...
  $(document).on('click', '#confirmSubmitTask', function () {
    window.parent.playClickEffect();
    var finishDesc = $('#finishDesc');
    errorTip(finishDesc);
    if (finishDesc.val()) {
      var title = "提交任务";
      var type = "1";
      var con = "您确认要提交任务吗？";
      var clickBtn = 'confirmSubmitTask';
      zPopup(title, type, con, clickBtn, id);
    }
  });
  //上传文件
  ...
  $(document).on("click", '.confirm', function () {
    ...
    $(".pitch.his_active").each(function () {
      ...
      $(".goldInput").each(function () {
        if ($(_this2).val() && !isInt($(_this2).val())) {
          checkOk = false;
  ...
  $(document).on('click', '#ratify', function () {
    ...
    if (user_id || parseInt(user_id) === 0) {
      ...
      if (leaderId) {
        ...
        var _title6 = "批准任务";
        var _type6 = "1";
        var _con6 = "您确认批准" + _name + "接取此任务吗？";
        var _clickBtn = 'retifyBtn';
        zPopup(_title6, _type6, _con6, _clickBtn, id, user_id);
      }
    }
  });
  /*二次确认弹窗*/
  ...
  $(document).on("click", ".formTeam", function () {
    var title = "创建队伍";
    var type = "1";
    var con = "队伍名称：<input type='text' placeholder='请输入队伍名称' class='nameBtn' />";
