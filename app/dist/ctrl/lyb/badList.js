...
$(function () {
  ...
  function getHitList() {
    service.getHitList(function (res) {
      ...
      $('#accountTable1').dataTable({
        ...
        data: res,
        columns: [{ data: 'num', title: '序号', orderable: false }, { data: 'user.name', title: '姓名', orderable: false }, { data: 'dataTime', title: '禁止接任务时间', orderable: false }, { data: 'reason', title: '说明', orderable: false }, { data: 'btn', title: '', orderable: false }]
      });
      deleteId = $("#id").data('id');
  ...
  $(document).on('click', '.btn-manager', function () {
    ...
    if (window.sessionStorage.getItem('role_id') == 4 || window.sessionStorage.getItem('role_id') == 5) {
      confirmAgain({
        ...
        btns: [{ domId: 'cancelAnswer', text: '取消' }, { domId: 'confirm', text: '确定', event: deleteMember }]
      });
    } else {
      confirmAgain({
        title: '移除黑名单',
  ...
  function deleteMember() {
    service.getDeleteBlackList(deleteId, function () {
      location.reload();
    });
  }
  getHitList();
