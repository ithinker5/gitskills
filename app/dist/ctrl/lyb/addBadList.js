...
$(function () {
  ...
  var badListObj = { user_ids: [], start_time: '', end_time: '', reason: '' };
  var savebadListObj = window.sessionStorage.getItem('badListObj');
  ...
  function getObj() {
    ...
    var desc = $('.allianceIntro').val();
    var deadlineEnd = $('#deadlineEnd').val();
    //let addName=$('.addName').text();
    if (deadlineStart) {
      badListObj.start_time = deadlineStart;
    }
    if (desc) {
      badListObj.reason = desc;
    }
    if (deadlineEnd) {
      badListObj.end_time = deadlineEnd;
    }
  ...
  $("#submit").click(function () {
    window.parent.playClickEffect();
    var deadlineStart = $('#deadlineStart');
    var desc = $('.allianceIntro');
    var deadlineEnd = $('#deadlineEnd');
    var addName = $('.addName');
    var addLeader = $('.addLeader');
    if (addName.val() != '') {
      ...
      service.getTime(function (res) {
        if (parseInt(res.now) <= parseInt(new Date($('#deadlineStart').val()).getTime())) {
          ...
          confirmAgain({
            ...
            btns: [{ domId: 'cancelAnswer', text: '取消' }, { domId: 'confirm', text: '确定', event: confirm }]
          });
        } else {
  ...
  function confirm() {
    service.getAddBlackList(badListObj, function (res) {
      if (res.status == 400) {
        confirmAgain({
          title: '提示',
          content: {
