...
jQuery(document).ready(function () {
  ...
  function getMember(id) {
    service.getMember(id, function (res) {
  ...
  function showall(menu_list, parent, obj) {
    $.each(menu_list, function (index, val) {
      ...
      if (val.deep === 1) {
        var html = '<div class="parentBubble"><div class="firstNode"><span class="title">盟主<span class="name">' + val.name + '</span></span>' + '<div class="head"><img src="' + val.avatar + '"/> </div>' + '<p class="intro">' + val.remark + '</p>' + '<span class="grade">Lv：' + val.level + '</span>' + '<img class="add" src="../i/space.png" /></div>';
        html += '<div class="bubbleBox dis_n" style="top:10rem;left: 36%;">' + '<div class="bubbleTitle"><span>帮众</span></div>' + '<div class="yqPerson">';
        ...
        if (val.role_id >= 4) {
          var _html = '<div class="parentBubble"><div class="secondNode"><span class="title">帮主<span class="name">' + val.name + '</span></span>' + '<div class="head"><img  src="' + val.avatar + '"/> </div>' + '<p class="intro">' + val.remark + '</p>' + '<span class="grade">Lv：' + val.level + '</span>' + '<img class="add" src="../i/space.png" /></div>';
          _html += '<div class="bubbleBox bottom80 dis_n">' + '<div class="bubbleTitle"><span>帮众</span></div>' + '<div class="yqPerson">';
          ...
          if (val.role_id === 3) {
            var _html2 = '<div class="parentBubble"><div class="secondNode"><span class="title">帮主<span class="name">' + val.name + '</span></span>' + '<div class="head"><img  src="' + val.avatar + '"/></div>' + '<p class="intro">' + (val.remark != null && val.remark.length > 0 ? val.remark : '无') + '</p>'
            /*+'<p class="intro">'+(val.remark==null?val.desc:'无')+'</p>'*/
            + '<span class="grade">Lv：' + val.level + '</span>' + '<img class="add" src="../i/space.png" /></div>';
            _html2 += '<div class="bubbleBox bottom80 dis_n">' + '<div class="bubbleTitle"><span>帮众</span></div>' + '<div class="yqPerson">';
            for (var _i2 = 0; _i2 < val.children.length; _i2++) {
              if (val.children[_i2].role_id === 1) {
                if (_i2 === parseInt(val.children.length) - 1) {
                  _html2 += "<span>" + val.children[_i2].name + "</span>";
                } else {
                  _html2 += "<span>" + val.children[_i2].name + "\uFF0C</span>";
                }
              }
            ...
            _html2 += '</div></div></div>';
            li.append(_html2).append('<ul></ul>').appendTo(parent);
          }
        }
      } else if (val.deep === 3) {
        var _html3 = '';
        if (val.role_id === 2) {
          _html3 = '<div class="parentBubble"><div class="thirdNode"><span class="title">' + (val.role_id == 2 ? '长老' : '帮众') + '<span class="name">' + val.name + '</span></span>' + '<div class="head"><img  src="' + val.avatar + '"/> </div>' + '<p class="intro">' + val.remark + '</p>' + '<span class="grade">Lv：' + val.level + '</span>' + '<img class="add" src="../i/space.png" /></div>';
          _html3 += '<div class="bubbleBox bottom80 dis_n">' + '<div class="bubbleTitle"><span>帮众</span></div>' + '<div class="yqPerson">';
          for (var _i3 = 0; _i3 < val.children.length; _i3++) {
            if (val.children[_i3].role_id === 1) {
              if (_i3 === parseInt(val.children.length) - 1) {
                _html3 += "<span>" + val.children[_i3].name + "</span>";
              } else {
                _html3 += "<span>" + val.children[_i3].name + "\uFF0C</span>";
              }
            }
          ...
          _html3 += '</div></div></div>';
          li.append(_html3).append('<ul></ul>').appendTo(parent);
        }
      }
      ...
      if (val.role_id === 5) {
        ...
        var $i = $('<div></div>');
        var html2 = '<i arrow="left" class="dis_n"><span class="title">' + '护法' + '<span class="name">' + obj.name + '</span></span>' + '<div class="head"><img  src="' + obj.avatar + '"/> </div>' + '<p class="intro">' + obj.remark + '</p>' + '<span class="grade">Lv：' + obj.level + '</span>' + '<img class="add" src="../i/space.png" /></i> ';
