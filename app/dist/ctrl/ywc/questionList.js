...
$(function () {
  ...
  function getList() {
    service.getQuestionList(function (res) {
      var $container = $('#tab1');
      for (var i = 0, len = res.length; i < len; i++) {
        ...
        switch (parseInt(res[i].status)) {
          case 0:
            res[i].statusChinese = '提问中';break;
          case 1:
            res[i].statusChinese = '已解答';break;
          case 2:
            res[i].statusChinese = '已关闭';break;
        }
      }
      $container.empty();
      ...
      $('#question').dataTable({
        ...
        data: res,
        columns: [{ data: 'num', title: '序号', orderable: true }, { data: 'title', title: '标题', orderable: true }, { data: 'author.name', title: '提问人', orderable: true }, { data: 'statusChinese', title: '状态', orderable: true }, { data: 'created_at', title: '发布时间', orderable: true }]
      });
