...
function getTangList() {
  var factionFilter = faction_id || '';
  service.getTangList(factionFilter, function (res) {
    var data = {
      listPage: true,
      list: res,
      nav: nav
...
function getUserList() {
  var factionFilter = faction_id || '';
  service.getUserList(factionFilter, function (res) {
    var data = {
      listPage: true,
      ...
      service.uploadFile('file', function (res) {
        if (res.error_code && parseInt(res.error_code) === 100001) {
          ...
          confirmAgain({
            ...
            btns: [{ domId: 'cancel', text: '确定', event: failData }]
          });
        } else {
          $(".feehideBox").empty();
          getUserList();
...
function getExpList(activePage) {
  var $activePage = activePage || 'level';
  if ($activePage === 'level') {
    service.getLevelList(function (res) {
      var listArr = {
        col0: [],
        col1: [],
        col2: [],
    ...
    service.getNewPersonTaskList(function (res) {
      var data = {
