...
jQuery(document).ready(function () {
  ...
  function getStructure1(id) {
    service.getStructure1(id, function (res) {
  ...
  function showall(menu_list, parent) {
    $.each(menu_list, function (index, val) {
      ...
      if (val.deep === 1) {
        var html1 = '<div class="bubbleBox dis_n">' + '<div class="bubbleTitle">';
        if (val.master == null) {
          html1 += '<span>帮主</span><span class="span2">无</span>' + '</div>' + '<p>人数：<span>' + val.member_count + '</span></p>';
        } else {
          html1 += '<span>帮主</span><span class="span2">' + val.master.name + '</span>' + '</div>' + '<p>人数：<span>' + val.member_count + '</span></p>';
        }
        html1 += '<p>帮会VC：<span>' + val.vc + '</span></p>' + '<p>帮会金币：<span>' + val.gold + '</span></p>' + '<p>部门组织架构：<span>' + val.org_info + '</span></p>' + '<p>简介：<span>' + val.desc + '</span></p>' + '</div>';
        ...
        html2 += '<p>人数：<span>' + val.member_count + '</span></p>' + '<p>简介：<span>' + val.desc + '</span></p>' + '</div>';
