...
$(function () {
  ...
  function getLeaders() {
    service.getMemberList(function (res) {
      ...
      $('#accountTable1').dataTable({
        ...
        data: res,
        columns: [{ data: 'num', title: '序号', orderable: true }, { data: 'work_num', title: '工号', orderable: true }, { data: 'name', title: '姓名', orderable: true }, { data: 'factions2', title: '所属帮会', orderable: true }, { data: 'role_id2', title: '职务', orderable: true }, { data: 'base_vc', title: '基础VC', orderable: true }, { data: 'task_vc2', title: '任务VC', orderable: true }, { data: 'gold', title: '金币', orderable: true }, { data: 'btn', title: '', orderable: false }]
      });
  ...
  $(document).on('click', '#confirmChange', function () {
    var number1 = $(this).parent('td').siblings().find('#base_vc');
      ...
      if (!isInt(number1.val())) {
        ...
        if (parseInt(departVc) + parseInt(baseVc) < parseInt(changeVc.base_vc)) {
          ...
          var changeMember = function changeMember() {
            service.getChangeMember(userId, changeVc, function () {
          ...
          confirmAgain({
            ...
            btns: [{ domId: 'cancelAnswer1', text: '取消' }, { domId: 'confirm', text: '确定', event: changeMember }]
          });
        }
      }
    }
  ...
  $(document).on('click', '.clearZero', function () {
    ...
    confirmAgain({
      ...
      btns: [{ domId: 'cancelAnswer', text: '取消' }, { domId: 'confirm', text: '确定', event: clearAll }]
    });
    function clearAll() {
      service.getClearing(function (res) {
        window.location.reload();
      });
    }
  });
    ...
    service.uploadFile('file', function (res) {
      if (res.error_code && parseInt(res.error_code) === 100001) {
        ...
        confirmAgain({
          ...
          btns: [{ domId: 'confirm', text: '确定', event: failData }]
        });
      } else {
        // $(".feehideBox").empty();
        window.location.reload();
  ...
  //导出
  function exportFile() {
    service.exportFile(function (res) {
      window.open(res);
      $(".feehideBox").html('');
    });
  }
