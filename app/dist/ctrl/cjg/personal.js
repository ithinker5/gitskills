...
$(function () {
  ...
  var PropertiesObj = { name: '', desc: '', value: '' };
  /*个人默认值*/
  ...
  $(document).on('click', '#confirmChangeD', function () {
    var number1 = $(this).parent('b').siblings().find('.base_limit');
    errorTip(number1);
    ...
    if (number1.val()) {
      if (!isInt(number1.val())) {
        ...
        var changeProperties = function changeProperties() {
          service.changeProperties(PropertiesObj, function () {
        ...
        confirmAgain({
          ...
          btns: [{ domId: 'cancelAnswer2', text: '取消' }, { domId: 'confirm', text: '确定', event: changeProperties }]
        });
      }
    }
  });
