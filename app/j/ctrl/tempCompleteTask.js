$(function () {
  ...
  function getTask(id) {
    service.getById(id, (res) => {
      ...
      let html2='<div><p><label>验收人：</label><span>'+(res.acceptance?res.acceptance:res.created.name)+'</span></p></div>'
      ...
      if(trialTask && trialTask == 'true'){
        html2+='<div><p><label>领队评分：</label><span>'+res.applicant.rating+'</span></p></div>'
          +'<div class="oneItem"><p><label>领队评价：</label><span>'+(res.applicant.comments ? res.applicant.comments : '无')+'</span></p></div>'
      ...
      let allVc=parseInt($(".base_vc").text())-parseInt($(".off_vc").text())+parseInt($(".extra_vc").text());
      let allGold=parseInt($(".base_gold").text())-parseInt($(".off_gold").text())+parseInt($(".extra_gold").text());
      $(".allVC").text(allVc);
      ...
      $(".awardBox #accountTable2").dataTable({
        ...
        columns: [
          {data: 'role_id2', title: '职务', orderable: false},
          {data: 'user.name', title: '姓名', orderable: false},
          {data: 'vc', title: '奖励VC', orderable: false},
          {data: 'gold', title: '奖励金币', orderable: false},
          {data: 'exp', title: '经验', orderable: false},
          {data: 'rating', title: '评分', orderable: false},
          {data: 'comments', title: '评价', orderable: false},
        ]
      });
      if(trialTask && trialTask == 'true'){
        $(".awardBox").hide();
