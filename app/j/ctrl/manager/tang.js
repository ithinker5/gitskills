$(function () {
  ...
  $(document).on('click','#deleteSection',function () {
    ...
    confirmAgain({
      ...
      btns:[
        {domId:'cancel',text:'取消'},
        {domId:'confirm',text:'确定',event:deleteTang},
      ]
    });
    function deleteTang() {
      service.deleteTang(currentTangId,res=>{
        getTangList()
      })
    }
  });
  function getInfoById(currentTangId,page) {
    service.getTangById(currentTangId,res=>{
      let data={
        tangObj:res
      };
      if(page==='detail'){
  ...
  function add() {
    let name=$('#nameAdd');
    let desc=$('#descAdd');
    let faction=$('#factionName').data('value');
    service.addTang({name:name.val(),desc:desc.val(),faction_id:faction}, res=>{
