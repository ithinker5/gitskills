  ...
  function getMyTeams() {
    ...
    service.getMyTeams(function (res) {
      for(var i=0,len=res.length;i<len;i++){
        ...
        let teamMember=[];
        for(let j=0,len=res[i].members.length;j<len;j++){
      ...
      $('#accountTable1').dataTable({
        ...
        columns: [
          ...
          {data: 'role',title: '担任角色',orderable: true},
          {data: 'member_count', title: '队伍人数', orderable: true},
          {data: 'task_num', title: '任务数', orderable: true},
          {data: 'teamMember',title: '队员',orderable: true},
          ]
      });
