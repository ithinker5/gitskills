    ...
    service.getById(id, (res) => {
      ...
      for (let i = 0; i < res.taskApplicants.length; i++) {
       ...
       /* if(window.sessionStorage.getItem('user_id') == res.taskApplicants[i].user._id){
          $("#footer").html('<input id="allocateAward" class="btn subBtn2" type="button" value="分配奖励"/>');
        }*/
        /*第二张表格input框样式*/
      ...
      let html2='<div><p><label>验收人：</label><span>'+(res.acceptance?res.acceptance:res.created.name)+'</span></p></div>'
        +'<div><p><label>验收结果：</label><span>失败</span></p></div>'
      if(trialTask && trialTask == 'true'){
        ...
        html2+='<div>'
          ...
          +'<p><label>扣除V&nbsp;C：</label><span class="off_vc">'+(parseInt(res.status)===14?res.base_vc:res.off_vc)+'</span></p>'
          +'<p><label>奖励V&nbsp;C：</label><span class="extra_vc">'+res.extra_vc+'</span></p>'
          +'</div>'
          +'<div>'
          +'<p><label>悬赏金币：</label><span class="base_gold">'+res.gold+'</span></p>'
          +'<p><label>扣除金币：</label><span class="off_gold">'+(parseInt(res.status)===14?res.gold:res.off_gold)+'</span></p>'
