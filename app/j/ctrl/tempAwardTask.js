$(function () {
  ...
  function getTask(id) {
    service.getById(id, (res) => {
      ...
      let html2='<div><p><label>验收人：</label><span>'+res.created.name+'</span></p></div>'
                +'<div><p><label>验收结果：</label><span>验收通过</span></p></div>'
          ...
          html2+='<div>'
          +'<p><label>悬赏V&nbsp;C：</label><span class="base_vc">'+res.base_vc+'</span></p>'
          +'<p><label>扣除V&nbsp;C：</label><span class="off_vc">'+res.off_vc+'</span></p>'
          +'<p><label>奖励V&nbsp;C：</label><span class="extra_vc">'+res.extra_vc+'</span></p>'
          +'</div>'
          +'<div>'
          +'<p><label>悬赏金币：</label><span class="base_gold">'+res.gold+'</span></p>'
          +'<p><label>扣除金币：</label><span class="off_gold">'+res.off_gold+'</span></p>'
          +'<p><label>奖励金币：</label><span class="extra_gold">'+res.extra_gold+'</span></p>'
          +'</div>'
          +'<div class="oneItem"><p><label>验收说明：</label><span>'+(res.check_desc ? res.check_desc : '无')+'</span></p></div>';
    ...
    service.getById(id, (res) => {
      ...
      $(".awardBox #accountTable2").dataTable({
        ...
        columns: [
          {data: 'role_id2', title: '职务', orderable: false},
          {data: 'user.name', title: '姓名', orderable: false},
          {data: 'awardVC', title: '奖励VC', orderable: false},
          {data: 'awardGold', title: '奖励金币', orderable: false},
          {data: 'exp', title: '经验', orderable: false},
          {data: 'graded', title: '评分', orderable: false},
          {data: 'evaluate', title: '评价', orderable: false},
        ],
      });
      /*监听输入框改变剩余奖励值数据*/
      $(".awardVC").bind('input propertychange',function(){
        let originVC=parseFloat(allVc);
      ...
      $(".awardGold").bind('input propertychange',function(){
        let originGold=parseFloat(allGold);
  ...
  $(document).on('click','#submit',function(){
    $("input.selest").each(function(){
        ...
        if(parseFloat(uploadVc)<parseInt(allVc)*0.2 ){
          ...
          if( parseFloat(uploadGold)<parseFloat(allGold)*0.2){
            ...
            /*if($(this).parent().parent().next().children(".awardInput2").val().length>15){
              ...
              if(checkOk){
                const title = '奖励分配';
                const con = '您确认要分配此任务奖励吗？';
                const type = '1';
                const clickBtn ='submitOk';
      ...
      let awardInfo={_id:$(this).data('id'),vc:uploadVc,gold:uploadGold,rating:$(this).val(),comments:$(this).parent().parent().next().children("#evaluate").val()};
      applyObj.push(awardInfo);
    });
  });
  $(document).on('click','.submitOk',function(){
    service.getCompleteTask(7,res => {});
    service.getAwardTask(id,applyObj,res=>{
      if(res.code==400){
        const title = "提示";
        const type = "2";
        const con = res.msg;
  ...
  $(document).on('click','.select_box input',function(){
    ...
    if (thisul.css("display") == "none") {
      thisul.fadeIn("100");
      let len=parseInt(($(".dataTables_scrollBody tbody").height())+parseInt($(".select_ul").height()))+'px';
