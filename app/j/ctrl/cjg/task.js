$(function(){
  ...
  let userId='';
  let changeVc={default_vc:'',default_gold:''};
  getTaskList();
  //获取领队列表
  function getTaskList() {
    service.getTaskList(function (res) {
      ...
      $('#accountTable1').dataTable({
        ...
        data: res,
        columns: [
          { data: 'type', title: '任务类型', orderable: false },
          { data: 'level', title: '级别', orderable: false },
          { data: 'default_vc', title: '默认VC值', orderable: false },
          { data: 'default_gold', title: '默认金币值', orderable: false },
  ...
  $(document).on('click','#confirmChange',function () {
    let number1=$(this).parent('td').siblings().find('#task_vc');
    ...
    if(number1.val()||number2.val()){
      if(!isInt(number1.val())){
        ...
        function changeMember(){
          service.changeTaskList(userId,changeVc, ()=>{
            //location.reload();
          })
        }
      }
