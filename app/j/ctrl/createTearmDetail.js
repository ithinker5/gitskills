    ...
    service.getTeamsList(id,function (res) {
      if(getUrlParam('roleId')==1){
        ...
        $('#accountTable1').dataTable({
          ...
          data:res,
          columns: [
            {data: 'num',title: '序号',orderable: false},
        ...
        /*队长详情*/
        for(let i=0,len=res.length;i<len;i++){
          user_Id.push(res[i].user._id);
          res[i].num=i+1;
          res[i].user.level ='Lv'+res[i].user.level;
          res[i].user.title=res[i].user.title||'无';
          if(window.sessionStorage.getItem('user_id')==res[i].user._id){
            res[i].deleteBtn ='<input class="deleteBtn dis_n" data-id="'+res[i]._tmid+'" value="删除" type="button"/>';
          }else{
        ...
        $('#accountTable1').dataTable({
          ...
          columns: [
            ...
            {data: 'user.title', title: '岗位名称', orderable: false},
            {data: 'user.grade',title: '岗位级别',orderable: false},
            {data: 'deleteBtn',title: '',orderable: false},
          ]
        });
      ...
      $("#quit").click(function(){
        ...
        const title = "退出队伍";
        const type= "1";
        const con = "您确认要退出此队伍吗？</br>您将失去所有押金！";
      ...
      $(".deleteBtn").click(function(){
        ...
        const title = "删除";
        const type= "1";
        const con = "您确认将此队员踢出队伍吗？</br>该队员将失去所有押金！";
  ...
  function getTeamsTasks(id) {
    service.getTeamsTasks(id,function (res) {
      ...
      $('#accountTable2').dataTable({
        ...
        columns: [
          {data: 'num', title: '序号', orderable: false},
          {data: 'name',title: '任务名称',orderable: false},
          {data: 'subtype2', title: '任务类型', orderable: false},
          {data: 'taskNum', title: '任务级别', orderable: false},
          {data: 'status2',title: '任务状态',orderable: false},
          {data: 'teamMember',title: '发布人',orderable: false},
  ...
  function zPopup(title,type, con,userId) {
    ...
    if(getUrlParam('roleId')==1){
      $(".gotoDelete").off("click").on("click",function(){
        ...
        service.getDeleteTeams(userId,res => {
