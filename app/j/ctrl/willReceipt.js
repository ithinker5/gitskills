...
//获取任务文档
  ...
  $(document).on('click', '#submitReceiptTask', function () {
    window.parent.playClickEffect();
    let obj={};
    let $status=parseInt($('input[name="result"]:checked').val());
    let originStatus=parseInt($('#status').data('status'));
    let receiptInfo=$('#receiptInfo');
    let leaderEvaluate=$('#leaderEvaluate');
    if($status===11){
      if(originStatus===6){
            ...
            if(!faction_id){
              if(personalTask && personalTask=='true'){
                if(parseInt($('#addVC').val())>parseInt(personalVC)){
                  ...
                  const title = "验收任务";
                  const type = "1";
                  const con = "您确认要验收此任务吗？";
                  const clickBtn = 'submitReceiptTask';
  ...
  $(document).on('click', '#confirmReSubmit', function () {
    window.parent.playClickEffect();
    let finishDesc=$('#finishDescWrite');
    errorTip(finishDesc);
    if(finishDesc.val()){
      const title = "提交任务";
      const type = "1";
      const con = "您确认要重新提交此任务吗？";
      const clickBtn = 'confirmReSubmit';
  ...
  function deleteFile() {
    let index=$('#finishFileContainerWrite .delete').index($(this));
  ...
  function zPopup(title, type, con, clickBtn, id, obj) {
    ...
    $(".confirmReSubmit").off("click").on("click", function () {
      window.parent.playClickEffect();
      let obj={status:6,complete_desc:$('#finishDescWrite').val()};
      obj.assets=fileArr;
